...
if __name__ == "__main__":
    ...
    print("  5. DISP kv : CUMULATIVE prompt tokens dispatched per backend, over ALL")
    print("               requests including rejections. Read it as 'did each backend")
    print("               do a fair share of total work', NOT as the mechanism.")
    print("               A cost-aware policy can score WORSE here while winning on")
    print("               err%, and that is not a contradiction: rejections are caused")
    print("               by INSTANTANEOUS occupancy, and a policy that routes to")
    print("               whoever has headroom right now produces uneven totals on")
    print("               purpose. Measured: at rate 8 pressure beat least_conn 0.3%")
    print("               to 2.0% on errors with HIGHER cumulative spread.")
    print("  6. DISP req: spread of request COUNTS dispatched. least_conn drives this")
    print("               toward 0 by construction.")
    print("  7. ok tok  : served tokens, successes only. Do NOT read it as balance --")
    print("               a backend that rejects most of its traffic looks lightly")
    print("               loaded here.")
    print()
    print("  For the MECHANISM, run occupancy_stats.py on the run log. Instantaneous")
    print("  occupancy is what causes rejections, and it is what these columns cannot")
    print("  show.")
    print()
